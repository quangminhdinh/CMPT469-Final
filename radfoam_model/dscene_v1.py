...
class DynamicFoamScene(torch.nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        print("v1")
        # print(f"\n____{self.__class__.__name__}'s initialization started_____")
        ...
        # self.t_attr_scale = nn.Parameter(
        #     torch.ones(
        #         self.num_init_points,
        #         1,
        #         device=self.device,
        ...
        # print(f"\n____{self.__class__.__name__}'s initialization ended_____\n")

    def get_xyz(self, t):
        t_var = t - self.t_center
        xyz = torch.zeros_like(self.primal_points)
        acc = 1
        for i in range(self.np):
            acc = acc * t_var.clone()
            xyz = xyz + acc * self.p_deform[:, (i * 3) : ((i + 1) * 3)].clone()
        return self.primal_points + xyz
    ...
    #     xyz = torch.zeros_like(self.primal_points)
    #     for i in range(self.np):
    #         # Use sine basis functions instead of polynomials
    #         basis = torch.sin(np.pi * i * t_var)
    #         xyz = xyz + basis * self.p_deform[:, (i * 3) : ((i + 1) * 3)].clone()
    #     return self.primal_points + xyz
    
    # def get_temporal_attr(self, attr, scale, t):
    # # ... existing code ...
    #     t_var = t - self.t_center
    #     # Softer falloff using sigmoid or smoother activation
    #     t_dist = t_var / (1 + torch.abs(scale))  # More stable scaling
    #     return attr * torch.sigmoid(-t_dist.pow(2))
    
    def get_temporal_attr(self, attr, scale, t):
        t_var = t - self.t_center
        t_dist = t_var / torch.exp(scale)
        return attr * torch.exp(-1 * t_dist.pow(2))
        # return attr * torch.exp(-F.elu(self.t_duration) * (t - self.t_center) ** 2)
    
    def get_fourier_attr(self, attr, t):
        _attr = attr.clone()
        for i in range(attr.shape[-1]):
            # _attr[:, i] = _attr[:, i] * torch.cos(2 * np.pi * (i + 1) * t / self.max_frames)
            _attr[:, i] = _attr[:, i] * torch.cos(2 * np.pi * torch.tensor(i + 1) * t )
        return _attr.clone()
    ...
    def permute_points(self, permutation):
        ...
        self.t_center = optimizable_tensors["t_center"]
        self.t_duration = optimizable_tensors["t_duration"]
        # self.t_attr_scale = optimizable_tensors["t_attr_scale"]
        self.p_deform = optimizable_tensors["p_deform"]

    def update_triangulation(self, t=None, rebuild=True, incremental=False):
        t = None
        if not self.primal_points.isfinite().all() and\
            not self.t_center.isfinite().all() and \
                not self.p_deform.isfinite().all():
            raise RuntimeError("NaN in points")

        needs_permute = False
        perturbation = 1e-6
        org_points = self.primal_points if t is None else self.get_xyz(t)
        del_points = org_points
        failures = 0
        acc = torch.zeros_like(del_points)
        while rebuild:
            if failures > 25:
            ...
            try:
                ...
                incremental = False
                with torch.no_grad():
                    acc = perturbation * torch.randn_like(self.primal_points)
                    del_points = (
                        org_points + acc
                    )
        ...
        if failures > 5:
            with torch.no_grad():
                self.primal_points += acc

        if needs_permute:
            perm = self.triangulation.permutation().to(torch.long)
            self.permute_points(perm)

        self.aabb_tree = radfoam.build_aabb_tree(
            self.primal_points if t is None else self.get_xyz(t)
        )
    ...
    def get_primal_density(self, t):
        return self.activation_scale * F.softplus(
            # self.density
            self.get_temporal_attr(self.density, self.t_duration, t)
            , beta=10)

    def get_primal_attributes(self, t):
        spatial_attr = torch.cat([self.att_dc, self.att_sh], dim=-1)
        # return spatial_attr 
        return self.get_fourier_attr(spatial_attr, t)
        # return self.get_temporal_attr(spatial_attr, self.t_attr_scale, t)

    def get_trace_data(self, t):
        points = self.get_xyz(t)
        attributes = torch.cat(
            [self.get_primal_attributes(t), self.get_primal_density(t)],
            dim=-1,
        ).to(self.attr_dtype)
    ...
    def update_viewer(self, viewer, t):
        points, attributes, point_adjacency, point_adjacency_offsets = (
            self.get_trace_data(t)
        )
        viewer.update_scene(
    ...
    def declare_optimizer(self, args, warmup, max_iterations):
        params = [
            ...
            {
                "params": self.t_center,
                "lr": args.temporal_center_lr_init,
                "name": "t_center",
            },
            {
                "params": self.t_duration,
                "lr": args.temporal_duration_lr_init,
                "name": "t_duration",
            },
            # {
            #     "params": self.t_attr_scale,
            #     "lr": args.temporal_attr_scale_lr_init,
            #     "name": "t_attr_scale",
            # },
            {
                "params": self.p_deform,
                "lr": args.deform_lr_init,
                "name": "p_deform",
            },
        ]
        ...
        self.xyz_deform_scheduler_args = get_cosine_lr_func(
            lr_init=args.deform_lr_init,
            lr_final=args.deform_lr_final,
            max_steps=args.freeze_points,
        )
        ...
        self.temporal_center_scheduler_args = get_cosine_lr_func(
            lr_init=args.temporal_center_lr_init,
            lr_final=args.temporal_center_lr_final,
            max_steps=max_iterations,
        )
        self.temporal_duration_scheduler_args = get_cosine_lr_func(
            lr_init=args.temporal_duration_lr_init,
            lr_final=args.temporal_duration_lr_final,
            max_steps=max_iterations,
        )
        self.temporal_attr_scale_scheduler_args = get_cosine_lr_func(
            lr_init=args.temporal_attr_scale_lr_init,
            lr_final=args.temporal_attr_scale_lr_final,
            max_steps=max_iterations,
        )
    ...
    def update_learning_rate(self, iteration):
        ...
        for param_group in self.optimizer.param_groups:
            if param_group["name"] == "primal_points":
                ...
                param_group["lr"] = lr
            elif param_group["name"] == "p_deform":
                lr = self.xyz_deform_scheduler_args(iteration)
                param_group["lr"] = lr
            elif param_group["name"] == "t_center":
                lr = self.temporal_center_scheduler_args(iteration)
                param_group["lr"] = lr
            # elif param_group["name"] == "t_attr_scale":
            #     lr = self.temporal_attr_scale_scheduler_args(iteration)
            #     param_group["lr"] = lr
            else:
                lr = self.temporal_duration_scheduler_args(iteration)
                param_group["lr"] = lr
    ...
    def prune_and_densify(
        self, t, point_error, point_contribution, upsample_factor=1.2
    ):
        with torch.no_grad():
            ...
            perturbation = 0.25 * (self.primal_points[farthest_neighbor] - self.primal_points)
            ...
            # print(primal_error_accum)
            # print("______________________")
            # print(cell_radius)
            # print("________________________")
            # print(num_sample_points)
            sampled_inds = torch.multinomial(
                primal_error_accum * cell_radius + 1e-12,
                num_sample_points,
                replacement=False,
            )
            sampled_points = (self.primal_points + perturbation)[sampled_inds]
            ...
            new_params = {
                ...
                "t_center": self.t_center[sampled_inds],
                "t_duration": self.t_duration[sampled_inds],
                # "t_attr_scale": self.t_attr_scale[sampled_inds],
                "p_deform": self.p_deform[sampled_inds],
            }
    ...
    def cache_gradient(self):
        self._primal_grd += self.primal_points.grad.clone()
        self._att_dc_grd += self.att_dc.grad.clone()
        self._att_sh_grd += self.att_sh.grad.clone()
        self._density_grd += self.density.grad.clone()

        self._t_center_grd += self.t_center.grad.clone()
        self._t_duration_grd += self.t_duration.grad.clone()
        # self._t_attr_scale_grd += self.t_attr_scale.grad.clone()
        self._p_deform_grd += self.p_deform.grad.clone()

    def zero_gradient_cache(self):
        self._primal_grd = torch.zeros_like(self.primal_points, requires_grad=False)
        self._att_dc_grd = torch.zeros_like(self.att_dc, requires_grad=False)
        self._att_sh_grd = torch.zeros_like(self.att_sh, requires_grad=False)
        self._density_grd = torch.zeros_like(self.density, requires_grad=False)

        self._t_center_grd = torch.zeros_like(self.t_center, requires_grad=False)
        self._t_duration_grd = torch.zeros_like(self.t_duration, requires_grad=False)
        # self._t_attr_scale_grd = torch.zeros_like(self.t_attr_scale, requires_grad=False)
        self._p_deform_grd = torch.zeros_like(self.p_deform, requires_grad=False)

    def set_batch_gradient(self, cnt):
        ratio = 1 / cnt
        self.primal_points.grad = self._primal_grd * ratio
        self.att_dc.grad = self._att_dc_grd * ratio
        self.att_sh.grad = self._att_sh_grd * ratio
        self.density.grad = self._density_grd * ratio
        
        self.t_center.grad = self._t_center_grd * ratio
        self.t_duration.grad = self._t_duration_grd * ratio
        # self.t_attr_scale.grad = self._t_attr_scale_grd * ratio
        self.p_deform.grad = self._p_deform_grd * ratio

    def collect_error_map(self, data_handler, tg, white_bkg=True, downsample=2):
        rays, rgbs = data_handler.rays, data_handler.rgbs
        ts = data_handler.times
        # rays = rays[ts == t]
        # rgbs = rgbs[ts == t]
        ...
        for i in range(min(rays.shape[0], self.max_frames)
            # rays.shape[0]
            ):
            ...
            if t != prev_t:
                self.update_triangulation(t, incremental=True)
                prev_t = t
            ...
            rgba_output, _, contribution, _, errbox = self.forward(
                ray_batch, t, return_contribution=True
            )
    ...
    def collect_error_map_cpu(self, data_iter, tg, white_bkg=True, downsample=2):
        point_error_accum = torch.zeros_like(self.primal_points[..., 0:1])
        point_contribution_accum = torch.zeros_like(self.primal_points[..., 0:1])
        rgb_loss = nn.L1Loss(reduction="none")
        prev_t = -1
        ...
        for i in range(self.max_frames):
            ray_batch, rgb_batch, t = next(data_iter)
        # for i, (ray_batch, rgb_batch, t) in enumerate(data_loader):
            ...
            d = torch.randint(0, downsample, (2,))
            ray_batch = ray_batch[:, d[0] :: downsample, d[1] :: downsample, :].to(self.device)
            rgb_batch = rgb_batch[:, d[0] :: downsample, d[1] :: downsample, :].to(self.device)
    ...
    def save_ply(self, ply_path, t=0):
        self.update_triangulation(t, incremental=True)
        points = self.get_xyz(t).detach().float().cpu().numpy()
        density = self.get_primal_density(t).detach().float().cpu().numpy()
        color_attributes = (
            self.get_primal_attributes(t).detach().float().cpu().numpy()
        )
    ...
    def save_pt(self, pt_path):
        ...
        t_center = self.t_center.detach().float().cpu()
        t_duration = self.t_duration.detach().float().cpu()
        # t_attr_scale = self.t_attr_scale.detach().float().cpu()
        p_deform = self.p_deform.detach().float().cpu()
    ...
    def load_pt(self, pt_path):
        ...
        self.t_center = nn.Parameter(
            scene_data["t_center"].to(self.attr_dtype).to(self.device)
        )
        self.t_duration = nn.Parameter(
            scene_data["t_duration"].to(self.attr_dtype).to(self.device)
        )
        # self.t_attr_scale = nn.Parameter(
        #     scene_data["t_attr_scale"].to(self.attr_dtype).to(self.device)
        # )
        self.p_deform = nn.Parameter(
            scene_data["p_deform"].to(self.attr_dtype).to(self.device)
        )
