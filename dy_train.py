...
def train(args, pipeline_args, model_args, optimizer_args, dataset_args):
    ...
    train_data_handler = DyDataHandler(
    ...
    def test_render(
        test_data_handler, tg = None, debug=False, iter=-1
    ):
        t = None
        rays, rgbs = test_data_handler.rays, test_data_handler.rgbs
        ts = test_data_handler.times
        if tg is not None:
            t = tg.to(ts.device)
            rays = rays[ts == t]
            rgbs = rgbs[ts == t]
        ...
        psnr_list = []
        with torch.no_grad():
            for i in tqdm.tqdm(range(rays.shape[0]), disable=debug):
    ...
    def train_loop(viewer):
        ...
        ray_batch, rgb_batch, alpha_batch, t_batch = next(data_iterator)
        ray_feat_dim, rgb_feat_dim, alpha_feat_dim = ray_batch.shape[-1], rgb_batch.shape[-1], alpha_batch.shape[-1]
        ray_batch = ray_batch.reshape(-1, ray_feat_dim)
        rgb_batch = rgb_batch.reshape(-1, rgb_feat_dim)
        alpha_batch = alpha_batch.reshape(-1, alpha_feat_dim)
        t = t_batch.squeeze()
        ...
        with tqdm.trange(pipeline_args.iterations) as train:
            for i in train:
                ...
                loss = color_loss.mean() + opacity_loss + 0.25 * Lssim #+ w_depth * quant_loss
                ...
                if i % 100 == 99 and not pipeline_args.debug:
                    ...
                    test_psnr = test_render(test_data_handler, None, True, i + 1)
                ...
                if (
                    ...
                    point_error, point_contribution = model.collect_error_map(
                        train_data_handler, t, pipeline_args.white_background
                    )
