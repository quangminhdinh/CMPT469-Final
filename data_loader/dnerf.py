...
class DNeRFDataset(Dataset):
    def __init__(
        self, datadir, split="train", is_stack=True
    ):
        f_path = os.path.join(datadir, f"transforms_{split}.json")
        with open(f_path, 'r') as samp_file:
            raw_f = json.load(samp_file)
        self.camera_angle_x = raw_f["camera_angle_x"]
        self.all_samples = raw_f["frames"]
